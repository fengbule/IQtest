...
- 支持上传照片生成精美形象报告
- 接入浏览器端 MediaPipe Face Mesh 人脸关键点模型，优先做脸型与五官几何分析
- 模型不可用或未检测到稳定人脸时，会自动降级为图片亮度、色温、比例等兜底分析
- 支持男女分开建议逻辑
- 结果包含脸型判断、五官重点、自动强化风格、适合使用场景、配色建议、发型建议、拍照建议
- 报告包含明星风格相似参考，用于公开人物风格路线参考，不代表身份识别或真实长相匹配
